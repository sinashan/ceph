...
int LSVDDriver::initialize(const DoutPrefixProvider* dpp)
{
...
int LSVDDriver::put(const DoutPrefixProvider* dpp, const std::string& key, const bufferlist& bl, uint64_t len, const rgw::sal::Attrs& attrs, optional_yield y)
{
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): key=" << key << dendl;
...
int LSVDDriver::get(const DoutPrefixProvider* dpp, const std::string& key, off_t offset, uint64_t len, bufferlist& bl, rgw::sal::Attrs& attrs, optional_yield y)
{
    char buffer[len];
...
int LSVDDriver::append_data(const DoutPrefixProvider* dpp, const::std::string& key, const bufferlist& bl_data, optional_yield y)
{
...
auto LSVDDriver::AsyncReadOp::create(const Executor1& ex1, CompletionHandler&& handler)
{
...
template <typename Executor1, typename CompletionHandler>
auto LSVDDriver::AsyncWriteRequest::create(const Executor1& ex1, CompletionHandler&& handler)
{
    auto p = Completion::create(ex1, std::move(handler));
    return p;
}
...
auto LSVDDriver::get_async(const DoutPrefixProvider *dpp, ExecutionContext& ctx, const std::string& key,
                off_t read_ofs, off_t read_len, CompletionToken&& token)
{
    std::string location = partition_info.location + key;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): location=" << location << dendl;
    ...
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): ::aio_read(), ret=" << ret << dendl;
    if(ret < 0) {
    ...
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): Before init.result.get()" << ret << dendl;
    return init.result.get();
}

template <typename ExecutionContext, typename CompletionToken>
void LSVDDriver::put_async(const DoutPrefixProvider *dpp, ExecutionContext& ctx, const std::string& key,
                const bufferlist& bl, uint64_t len, const rgw::sal::Attrs& attrs, CompletionToken&& token)
{
    ...
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): temp key=" << temp_key << dendl;
    r = op.prepare_libaio_write_op(dpp, src, len, temp_key, partition_info.location);
    op.cb->aio_sigevent.sigev_notify = SIGEV_THREAD;
    op.cb->aio_sigevent.sigev_notify_function = LSVDDriver::AsyncWriteRequest::libaio_write_cb;
    ...
    if (r >= 0) {
        ...
        ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): ::prepare_libaio_write_op(), r=" << r << dendl;
    }

    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): ::aio_write(), r=" << r << dendl;
    if(r < 0) {
...
rgw::Aio::OpFunc LSVDDriver::lsvd_cache_read_op(const DoutPrefixProvider *dpp, optional_yield y, rgw::cache::CacheDriver* cache_driver,
                                off_t read_ofs, off_t read_len, const std::string& key) {
  return [this, dpp, y, read_ofs, read_len, key] (Aio* aio, AioResult& r) mutable {
    ceph_assert(y);
    ldpp_dout(dpp, 20) << "LSVDCache: cache_read_op(): Read From Cache, oid=" << r.obj.oid << dendl;
    ...
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): key=" << key << dendl;
    this->get_async(dpp, y.get_io_context(), key, read_ofs, read_len, bind_executor(ex, LSVDDriver::libaio_read_handler{aio, r}));
  };
}

rgw::Aio::OpFunc LSVDDriver::lsvd_cache_write_op(const DoutPrefixProvider *dpp, optional_yield y, rgw::cache::CacheDriver* cache_driver,
  ...
  return [this, dpp, y, bl, len, attrs, key] (Aio* aio, AioResult& r) mutable {
    ceph_assert(y);
    ldpp_dout(dpp, 20) << "LSVDCache: cache_write_op(): Write to Cache, oid=" << r.obj.oid << dendl;
    ...
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): key=" << key << dendl;
    this->put_async(dpp, y.get_io_context(), key, bl, len, attrs, bind_executor(ex, LSVDDriver::libaio_write_handler{aio, r}));
  };
}

rgw::AioResultList LSVDDriver::get_async(const DoutPrefixProvider* dpp, optional_yield y, rgw::Aio* aio, const std::string& key, off_t ofs, uint64_t len, uint64_t cost, uint64_t id)
{
    rgw_raw_obj r_obj;
    r_obj.oid = key;
    return aio->get(r_obj, lsvd_cache_read_op(dpp, y, this, ofs, len, key), cost, id);
}

rgw::AioResultList LSVDDriver::put_async(const DoutPrefixProvider* dpp, optional_yield y, rgw::Aio* aio, const std::string& key, const bufferlist& bl, uint64_t len, const rgw::sal::Attrs& attrs, uint64_t cost, uint64_t id)
{
    rgw_raw_obj r_obj;
    r_obj.oid = key;
    return aio->get(r_obj, lsvd_cache_write_op(dpp, y, this, bl, len, attrs, key), cost, id);
}

int LSVDDriver::delete_data(const DoutPrefixProvider* dpp, const::std::string& key, optional_yield y)
{
    std::string location = partition_info.location + key;
...
int LSVDDriver::rename(const DoutPrefixProvider* dpp, const::std::string& oldKey, const::std::string& newKey, optional_yield y)
{
    std::string location = partition_info.location;

    int ret = std::rename((location+oldKey).c_str(), (location+newKey).c_str());
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "LSVDDriver: ERROR: failed to rename the file: " << location+oldKey << dendl;
        return ret;
    }
...
int LSVDDriver::AsyncWriteRequest::prepare_libaio_write_op(const DoutPrefixProvider *dpp, bufferlist& bl, unsigned int len, std::string key, std::string cache_location)
{
    std::string location = cache_location + key;
    int r = 0;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): Write To Cache, location=" << location << dendl;
...
void LSVDDriver::AsyncWriteRequest::libaio_write_cb(sigval sigval) {
...
int LSVDDriver::AsyncReadOp::prepare_libaio_read_op(const DoutPrefixProvider *dpp, const std::string& file_path, off_t read_ofs, off_t read_len, void* arg)
{
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): file_path=" << file_path << dendl;
    ...
    if(aio_cb->aio_fildes < 0) {
        int err = errno;
        ldpp_dout(dpp, 1) << "ERROR: LSVDCache: " << __func__ << "(): can't open " << file_path << " : " << " error: " << err << dendl;
        return -err;
    }
    if (dpp->get_cct()->_conf->rgw_d4n_l1_fadvise != POSIX_FADV_NORMAL) {
...
void LSVDDriver::AsyncReadOp::libaio_cb_aio_dispatch(sigval sigval)
{
    auto p = std::unique_ptr<Completion>{static_cast<Completion*>(sigval.sival_ptr)};
    auto op = std::move(p->user_data);
...
int LSVDDriver::update_attrs(const DoutPrefixProvider* dpp, const std::string& key, const rgw::sal::Attrs& attrs, optional_yield y)
{
    std::string location = partition_info.location + key;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): location=" << location << dendl;
    ...
    for (auto& it : attrs) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "LSVDCache: " << __func__ << "(): could not modify attr value for attr name: " << attr_name << " key: " << key << " ERROR: " << cpp_strerror(errno) <<dendl;
            return ret;
        }
    }
    efs::space_info space = efs::space(partition_info.location);
...
int LSVDDriver::delete_attrs(const DoutPrefixProvider* dpp, const std::string& key, rgw::sal::Attrs& del_attrs, optional_yield y)
{
    std::string location = partition_info.location + key;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): location=" << location << dendl;
    ...
    for (auto& it : del_attrs) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "LSVDCache: " << __func__ << "(): could not remove attr value for attr name: " << it.first << " key: " << key << cpp_strerror(errno) << dendl;
            return ret;
        }
    }
...
int LSVDDriver::get_attrs(const DoutPrefixProvider* dpp, const std::string& key, rgw::sal::Attrs& attrs, optional_yield y)
{
    std::string location = partition_info.location + key;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): location=" << location << dendl;
...
int LSVDDriver::set_attrs(const DoutPrefixProvider* dpp, const std::string& key, const rgw::sal::Attrs& attrs, optional_yield y)
{
    std::string location = partition_info.location + key;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): location=" << location << dendl;

    for (auto& [attr_name, attr_val_bl] : attrs) {
        ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): attr_name = " << attr_name << " attr_val_bl length: " << attr_val_bl.length() << dendl;
        if (attr_val_bl.length() != 0) {
            auto ret = set_attr(dpp, key, attr_name, attr_val_bl.to_str(), y);
            if (ret < 0) {
                ldpp_dout(dpp, 0) << "LSVDCache: " << __func__ << "(): could not set attr value for attr name: " << attr_name << " key: " << key << cpp_strerror(errno) << dendl;
                return ret;
            }
        }
    }
...
int LSVDDriver::get_attr(const DoutPrefixProvider* dpp, const std::string& key, const std::string& attr_name, std::string& attr_val, optional_yield y)
{
    std::string location = partition_info.location + key;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): location=" << location << dendl;

    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): get_attr: key: " << attr_name << dendl;
    ...
    if (attr_size < 0) {
        ldpp_dout(dpp, 0) << "LSVDCache: " << __func__ << "(): could not get attr value for attr name: " << attr_name << " key: " << key << dendl;
        attr_val = "";
        return errno;
    }
...
int LSVDDriver::set_attr(const DoutPrefixProvider* dpp, const std::string& key, const std::string& attr_name, const std::string& attr_val, optional_yield y)
{
    std::string location = partition_info.location + key;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): location=" << location << dendl;

    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): set_attr: key: " << attr_name << " val: " << attr_val << dendl;
...
int LSVDDriver::delete_attr(const DoutPrefixProvider* dpp, const std::string& key, const std::string& attr_name)
{
    std::string location = partition_info.location + key;
    ldpp_dout(dpp, 20) << "LSVDCache: " << __func__ << "(): location=" << location << dendl;
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "LSVDCache: " << __func__ << "(): could not remove attr value for attr name: " << attr_name << " key: " << key << cpp_strerror(errno) << dendl;
        return ret;
    }
